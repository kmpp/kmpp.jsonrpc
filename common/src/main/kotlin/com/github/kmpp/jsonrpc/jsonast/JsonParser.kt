...
internal class Parser(val source: String) {
    ...
    fun takeStr(): String {
        if (tc != TC_OTHER && tc != TC_STRING) fail(
            tokenPos,
    ...
    private fun nextString(source: String, startPos: Int) {
        ...
        val maxLen = source.length
        parse@ while (true) {
            if (curPos >= maxLen) fail(
                curPos,
                "Unexpected end in string"
            )
    ...
    private fun appendEsc(source: String, startPos: Int): Int {
        ...
        require(
            ...
            curPos
        ) { "Unexpected end after escape char" }
